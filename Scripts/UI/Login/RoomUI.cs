...
namespace PvpTank
{
    ...
    public partial class LobbyUI : MonoBehaviour
    {
        ...
        public void RequestMatch() //recordObject.activeSelf ||
        {
            ...
            else
            {
                foreach (var tab in matchInfotabList)
                {
                    if (tab.IsOn() == true)
                    {
                        MatchManager.GetInstance().RequestMatchMaking(tab.index);
                        return;
                    }
                }
       ...
       /* public void SetFriendList()
        {
            ClearFriendList();
            MatchManager.GetInstance().GetFriendList((bool result, List<Friend> friendList) =>
            {
...
*/
        ...
        public void InsertReadyUserPrefab(string nickName)
        {
            ...
            GameObject user = GameObject.Instantiate(friendPrefab, Vector3.zero, Quaternion.identity, readyUserListParent.transform);
